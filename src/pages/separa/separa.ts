...
@IonicPage()
@Component({
  selector: 'page-separa',
  templateUrl: 'separa.html',
})
export class SeparaPage {

  ionViewDidLoad() {
    console.log('ionViewDidLoad SeparaPage');
  }
  ...
  constructor(
    ...
    //CRIA PASTA APP_ROMANEIO NO DEVICE, SE A MESMA NÃO EXISTIR
    // this.file.createDir(this.file.externalRootDirectory, 'App_Romaneio', false).then(
    //  _ => this.toastCtrl.create({
    //    message: "DIRETÓRIO APP_ROMANEIO CRIADO COM SUCESSO",
    //    duration: 2000,
    //    position: 'top'
    // }).present()).catch(err => this.toastCtrl.create({
    //
    // }).present());
        ...
        buttons: [
                  ...
                  this.file.resolveLocalFilesystemUrl(uri).then(url => {
                    ...
                    this.file.readAsText(this.file.externalRootDirectory, 'Download/' + url.name).then(value => {
                      ...
                      ped3.forEach(element => {
                        ...
                        this.lista.push({ codigo: linha[0], nome: linha[1], prateleira: linha[2], quantidade: linhaCorrigida, qtd_separada: 0, qtd_estoqueTotal: linhaCorrigida2, lidoManual: false })
                      });



                      //-----------------------------GRAVANDO LISTA NO STORAGE-----------------------------------------------------------------
                      this.listaStorage.setItem('LISTA', this.lista)
                        .then(
                      ...
                      this.listaStorage.setItem('ROMANEIO', this.romaneio)
                        .then(
                          () => console.log('Stored item!'),
                          error => alert('Lista não gravada na memória interna -> ' + error)
                        );
                      ...
                      this.listaStorage.setItem('ALMOXARIFADO', this.almoxarifado)
                        .then(
                          () => console.log('Stored item!'),
                          error => alert('Lista não gravada na memória interna -> ' + error)
                        );
                      ...
                      this.listaStorage.setItem('EMPREITEIRA', this.empreiteira)
                        .then(
                          () => console.log('Stored item!'),
                          error => alert('Lista não gravada na memória interna -> ' + error)
                        );
...

          {
            ...
            handler: () => {
              ...
              if (contagemTerminada) {
                ...
                this.lista.forEach(element => {
                  this.mensagem.push('\n\n' + element.codigo + ' ' + element.nome + '\n quantidade separada -> ' + element.qtd_separada);
                });
                //transforma um array em uma única string para ser enviado pelo email
                ...
                let email = {
                  ...
                  bcc: [],
                  attachments: [],
                  subject: 'Romaneio n°' + this.romaneio + "  separado com sucesso " + " Colaborador: " + this.usuario[0].usuario + "\n" + " Registro: " + this.usuario[0].registro,
                  body: "Todos os materiais foram separados na sua totalidade co sucesso!!!!" +
                ...
                let alert = this.alertCtrl.create(
                  {
                    title: 'OS PEDIDOS DESSE ROMANEIO NÃO ESTÃO ZERADOS',
                    message: 'Você tem certeza de que deseja enviar email sem término da contagem? Existem materiais que  foram separados em quantidades diferentes a do romaneio.',
                    buttons: [
                      //---botao 1
                      {
                        text: "CANCELAR",
                        ...
                        handler: () => {
                          ...
                          materialDivergente.forEach(element => {
                            ...
                            if (element.quantidade > 0) {
                              ...
                              this.mensagem.push('\nCódigo: ' + element.codigo + ' faltou -> ' + falta + ' unidade(s) .\n\n');
                            }
                          ...
                          materialDivergente.forEach(element => {
                            if (element.quantidade < 0) {
                              ...
                              this.mensagem.push('\nCódigo: ' + element.codigo + '  separados à mais -> ' + sobra + ' unidade(s) .\n\n');
                            }
                          });
                          //transforma um array em uma única string para ser enviado pelo email
                          ...
                          let email = {
                            ...
                            bcc: [],
                            attachments: [],
                            subject: 'Romaneio n°' + this.romaneio + " COM DIVERGÊNCIAS " + " Colaborador: " + this.usuario[0].usuario + "\n" + " Registro: " + this.usuario[0].registro,
...

          {
            ...
            handler: () => {
              ...
              this.listaStorage.getItem('LISTA')
                .then(
              ...
              this.listaStorage.getItem('ROMANEIO')
                .then(
                  data => this.romaneio = data,
                  error => console.error(error)
                );

              this.listaStorage.getItem('ALMOXARIFADO')
                .then(
                  data => this.almoxarifado = data,
                  error => console.error(error)
                );

              this.listaStorage.getItem('EMPREITEIRA')
                .then(
      ...
      if (temCodigo) {
        ...
        if (temQuantidade) {
          ...
          if (temSerie) {
            ...
            if (temGedis) {
              ...
              if (codigoClicado.replace(/\s/g, "") == codigoQrcode) {
                ...
                if (qtdEstoqueTotal_clicado >= qtdQrcode) {
                  ...
                  if (qtdClicado >= qtdQrcode) {
                    ...
                    if (gedisNovo) {
                      this.lista.forEach(element => {
                        if (element.codigo.replace(/\s/g, "") == codigoQrcode && element.prateleira.replace(/\s/g, "") == prateleira_clicada) {
            ...
            } else {///começo tem serie e não tem gedis
              ...
              if (codigoClicado.replace(/\s/g, "") == codigoQrcode) {
                ...
                if (qtdEstoqueTotal_clicado >= qtdQrcode) {
                  ...
                  this.series.forEach(element => {   //verifica se existe um numero de serie igual ja lido
          ...
          } else {/// inicio tem código e quantidade apenas
          ...
          if (codigoClicado.replace(/\s/g, "") == codigoQrcode) {
            ...
            let alertQuantidade = this.alertCtrl.create(
              {
                title: "QUANTIDADE DO MATERIAL " + codigoClicado,
                message: "Insira manualmente a quantidade do material . Atenção, faltam separar " + qtdClicado + " unidade(s)",
                inputs: [
                  {
                    name: 'quantiadde',
                ...
                buttons: [
                  {
                    ...
                    handler: data => {
                      ...
                      if (qtdEstoqueTotal_clicado >= qtdDigitado) {
                        ...
                        alert('QUANTIDADE DE ' + qtdDigitado + ' MAIOR QUE A DISPONIVEL NO DEPÓSITO.');
                        repete = false;
                      }
                      if (repete) {
                        this.lerQr(item);
        ...
        buttons: [
          ...
          //---botao 2
          {
            ...
            handler: () => {
              ...
              this.lista.forEach(element => {
                if (element.codigo == codigoClicado && element.prateleira == prateleiraClicada && element.qtd_separada > 0) {
